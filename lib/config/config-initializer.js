...
function installModules(config) {
    ...
    if (config.plugins) {
        Object.keys(config.plugins).forEach(function (name) {
            modules[`eslint-plugin-${name}`] = config.plugins[name];
        });
    }
    if (config.extends && config.extends.indexOf("eslint:") === -1) {
        modules[`eslint-config-${config.extends}`] = "*";
    }
...
function getConfigForStyleGuide(guide) {
    ...
    const peerDeps = JSON.parse(execSync('npm info eslint-config-' + guide + '@latest peerDependencies --json'));
    const peerDepPlugins = Object.keys(peerDeps).filter(function (key) {
        return key.startsWith('eslint-plugin-') || key === 'eslint';
    }).reduce(function (acc, key) {
        acc[key] = peerDeps[key];
        return acc;
    }, {});

    installModules(Object.assign({}, guides[guide], { plugins: peerDepPlugins }));
